...
const Header = () => {
  ...
  // Check if we're on payment page - don't fetch dentist info there
  const isPaymentPage = location.pathname.includes('/subscription') || 
                        location.pathname === '/dashboard' && !dentistSurname;
  ...
  useEffect(() => {
    const fetchDentistInfo = async () => {
      // Skip fetching if we're on payment page (subscription endpoints are protected)
      ...
      try {
        ...
        // Silently fail - might be due to inactive subscription
        // Only log if it's not a 403 (subscription required) error
