...
import { User, Lock, Eye, EyeOff, ChevronLeft, Calendar, Building2, Phone } from 'lucide-react';
...
const PatientSignUp = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await patientAuthService.signup({
    ...
    >
      ...
      <div className="w-full max-w-md relative py-8 px-6 bg-white/90 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/40">
        ...
        <div className="mb-8 relative" style={{ zIndex: 10 }}>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">{t('signUpTitle')}</h1>
          <p className="text-gray-700 text-sm">{t('signUpSubtitle')}</p>
        </div>
        ...
        <div className="mt-6 text-center relative" style={{ zIndex: 10 }}>
          <p className="text-gray-600 text-sm">
            {t('hasAccount')}{' '}
            <button
              onClick={() => navigate(`/patient/login${formData.clinicId ? `?clinicId=${formData.clinicId}` : ''}`)}
