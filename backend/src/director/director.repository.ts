...
export class DirectorRepository {
  ...
  private get repo(): Repository<Director> {
    return this.dataSource.getRepository(Director);
  }
  ...
  async findForDentist(
    ...
    const qb = this.repo
      ...
      .innerJoinAndSelect('director.staff', 'staff')
      .where('staff.clinicId = :clinicId', { clinicId });

    if (filters.id !== undefined)
      qb.andWhere('director.id = :id', { id: filters.id });
    if (filters.staffId !== undefined)
      qb.andWhere('director.staffId = :staffId', { staffId: filters.staffId });
  ...
  async deleteForDentist(dentistId: number, id: number): Promise<void> {
    ...
    const existing = await this.repo
      .createQueryBuilder('director')
      .innerJoin('director.staff', 'staff')
      .where('director.id = :id', { id })
      .andWhere('staff.clinicId = :clinicId', { clinicId })
      .getOne();
    if (!existing) throw new Error('Forbidden');
