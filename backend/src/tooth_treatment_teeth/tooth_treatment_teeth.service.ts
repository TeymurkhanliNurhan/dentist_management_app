...
export class ToothTreatmentTeethService {
    ...
    async addTeeth(dentistId: number, dto: CreateToothTreatmentTeethDto) {
        try {
            ...
            if (e?.message?.includes('Forbidden')) {
                ...
                throw new BadRequestException("You don't have access to this tooth treatment");
            }
            throw new BadRequestException('Failed to add teeth to treatment');
        }
    }

    async removeTeeth(dentistId: number, toothTreatmentId: number, toothIds: number[]) {
        try {
    ...
    async getTeethForTreatment(dentistId: number, toothTreatmentId: number) {
        try {
            const records = await this.repo.getTeethForTreatment(dentistId, toothTreatmentId);
