...
export class ToothTreatmentTeethRepository {
    ...
    async removeTeethFromTreatment(
        ...
        const queryBuilder = this.repo
            .createQueryBuilder()
            .delete()
            .where('tooth_treatment_id = :toothTreatmentId', { toothTreatmentId: input.toothTreatmentId })
    ...
    async findAll(
        dentistId: number,
        filters: { id?: number; toothTreatmentId?: number; toothId?: number; patientId?: number },
    ): Promise<ToothTreatmentTeeth[]> {
        const queryBuilder = this.repo
        ...
        if (filters.toothId !== undefined) {
            queryBuilder.andWhere('patientTooth.tooth = :toothId', { toothId: filters.toothId });
        }
