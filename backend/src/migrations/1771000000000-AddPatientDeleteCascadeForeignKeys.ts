...
export class AddPatientDeleteCascadeForeignKeys1771000000000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await this.recreateToothTreatmentTeethPatientToothForeignKey(
      queryRunner,
      'CASCADE',
    );
  }
  ...
  private async recreateAppointmentPatientForeignKey(
    ...
    const fk = table.foreignKeys.find(
      (x) =>
        x.columnNames.length === 1 &&
        x.columnNames[0] === 'patient' &&
        x.referencedTableName === 'Patient',
    );
    if (fk) {
  ...
  private async recreatePatientTeethPatientForeignKey(
    queryRunner: QueryRunner,
    onDelete: 'CASCADE' | 'NO ACTION',
  ): Promise<void> {
