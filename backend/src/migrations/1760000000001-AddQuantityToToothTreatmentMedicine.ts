...
export class AddQuantityToToothTreatmentMedicine1760000000001
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      'ALTER TABLE "Tooth_Treatment_Medicine" ADD COLUMN IF NOT EXISTS "quantity" int NOT NULL DEFAULT 1',
    );
    await queryRunner.query(
      'UPDATE "Tooth_Treatment_Medicine" SET "quantity" = 1 WHERE "quantity" IS NULL',
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      'ALTER TABLE "Tooth_Treatment_Medicine" DROP COLUMN IF EXISTS "quantity"',
    );
  }
}
