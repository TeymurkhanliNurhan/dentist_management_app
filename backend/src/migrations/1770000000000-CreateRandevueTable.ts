...
export class CreateRandevueTable1770000000000 implements MigrationInterface {
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        if (table) {
            const fkAppointment = table.foreignKeys.find((fk) => fk.columnNames.indexOf('appointment') !== -1);
            if (fkAppointment) {
                await queryRunner.dropForeignKey('Randevue', fkAppointment);
            }
            table = await queryRunner.getTable('Randevue');
            if (table) {
                const fkPatient = table.foreignKeys.find((fk) => fk.columnNames.indexOf('patient') !== -1);
                if (fkPatient) {
