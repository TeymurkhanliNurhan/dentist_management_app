...
export class SeedClinic11May25WeekData1777700000000
  implements MigrationInterface
{
  name = 'SeedClinic11May25WeekData1777700000000';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      ...
      staff13_dentist_rn AS (
        SELECT dr.dentist_rn
        FROM dentist_rows dr
        INNER JOIN "Dentist" d ON d.id = dr.dentist_id
        INNER JOIN "Staff" st ON st.id = d."staffId" AND st.id = 13
        LIMIT 1
      ),
      dentist_count AS (
      ...
      base_seed AS (
        SELECT
          ...
          a.seq_rn,
          ('CL11-MAYWEEK-TT-' || a.seq_rn::text) AS description,
          ((a.seq_rn - 1) % (SELECT total FROM dentist_count)) + 1 AS assigned_dentist_rn
        FROM appointment_rows a
      ),
      base_seed_filtered AS (
        SELECT
          b.appointment_id,
          b.patient_id,
          b.seq_rn,
          b.description
        FROM base_seed b
        WHERE b.assigned_dentist_rn IS DISTINCT FROM (SELECT dentist_rn FROM staff13_dentist_rn)
           OR ((b.seq_rn - 1) % 10) < 7
      ),
      extra_s13_seed AS (
        SELECT
          ...
          a."patient" AS patient_id,
          a.seq_rn,
          ('CL11-MAYWEEK-S13-TT-' || a.seq_rn::text) AS description
        FROM appointment_rows a
        WHERE EXISTS (SELECT 1 FROM staff13_dentist_rn)
          AND a.seq_rn <= 19
      ),
      seed_rows AS (
        SELECT appointment_id, patient_id, seq_rn, description FROM base_seed_filtered
        UNION ALL
        SELECT appointment_id, patient_id, seq_rn, description FROM extra_s13_seed
      )
      ...
      INNER JOIN patient_teeth pt
       ...
       AND pt.tooth_rn = CASE
         WHEN s.description LIKE 'CL11-MAYWEEK-S13-TT-%' THEN 2
         ELSE 1
       END
      ...
      INNER JOIN dentist_rows dr
        ON dr.dentist_rn = CASE
          WHEN s.description LIKE 'CL11-MAYWEEK-S13-TT-%'
            THEN (SELECT dentist_rn FROM staff13_dentist_rn)
    ...
    await queryRunner.query(`
      WITH tt_rows AS (
        ...
        WHERE "description" LIKE 'CL11-MAYWEEK-TT-%'
           OR "description" LIKE 'CL11-MAYWEEK-S13-TT-%'
      )
    ...
    await queryRunner.query(`
      ...
      tt_rows AS (
        ...
        FROM "Tooth_Treatment" tt
        WHERE tt."description" LIKE 'CL11-MAYWEEK-TT-%'
           OR tt."description" LIKE 'CL11-MAYWEEK-S13-TT-%'
      ),
      appointment_rows AS (
        SELECT a.id AS appointment_id, a."startDate" AS appt_date
        FROM "Appointment" a
        INNER JOIN clinic_row c ON c.clinic_id = a."clinicId"
        WHERE a."startDate" BETWEEN '2026-05-25'::date AND '2026-05-31'::date
      )
    ...
    await queryRunner.query(`
      ...
      rv_rows AS (
        SELECT
          ...
          rv.note
        FROM "Randevue" rv
        WHERE rv.note LIKE 'CL11-MAYWEEK-RV-%'
           OR rv.note LIKE 'CL11-MAYWEEK-S13-RV-%'
      )
