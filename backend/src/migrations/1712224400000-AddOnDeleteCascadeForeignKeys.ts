 ...
 */
export class AddOnDeleteCascadeForeignKeys1712224400000
  implements MigrationInterface
{
  ...
  private async ensureToothTreatmentMedicineCascade(
    queryRunner: QueryRunner,
  ): Promise<void> {
    const table = await queryRunner.getTable('Tooth_Treatment_Medicine');
    if (!table) return;

    const fk = table.foreignKeys.find(
      (x) =>
        x.columnNames.includes('Tooth_Treatment') &&
        x.referencedTableName === 'Tooth_Treatment',
    );
    if (!fk) {
  ...
  private async ensureToothTreatmentAppointmentCascade(
    queryRunner: QueryRunner,
  ): Promise<void> {
    ...
    const fk = table.foreignKeys.find(
      (x) =>
        x.columnNames.includes('appointment') &&
        x.referencedTableName === 'Appointment',
    );
    if (!fk) {
      await queryRunner.createForeignKey(
        'Tooth_Treatment',
  ...
  private async ensureToothTreatmentTreatmentCascade(
    queryRunner: QueryRunner,
  ): Promise<void> {
    const table = await queryRunner.getTable('Tooth_Treatment');
    if (!table) return;
    ...
    const fk = table.foreignKeys.find(
      (x) =>
        x.columnNames.includes('treatment') &&
        x.referencedTableName === 'Treatment',
    );
    if (!fk) {
      await queryRunner.createForeignKey(
        'Tooth_Treatment',
  ...
  private async ensureMediaToothTreatmentCascade(
    queryRunner: QueryRunner,
  ): Promise<void> {
    ...
    const fk = table.foreignKeys.find(
      (x) =>
        x.columnNames.includes('Tooth_Treatment_id') &&
        x.referencedTableName === 'Tooth_Treatment',
    );
    if (!fk) {
      await queryRunner.createForeignKey(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const col of ['appointment', 'treatment'] as const) {
      ...
      const fk = tt?.foreignKeys.find(
        (x) =>
          x.columnNames.includes(col) && x.referencedTableName === refTable,
      );
      if (fk) {
        await queryRunner.dropForeignKey('Tooth_Treatment', fk);
        await queryRunner.createForeignKey(
