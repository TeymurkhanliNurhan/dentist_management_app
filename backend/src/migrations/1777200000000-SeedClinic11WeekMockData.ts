...
export class SeedClinic11WeekMockData1777200000000
  implements MigrationInterface
{
  name = 'SeedClinic11WeekMockData1777200000000';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ...
      SELECT seed."startDate"::date, seed."endDate"::date, seed."calculatedFee",
             seed."chargedFee", seed."discountFee", c.clinic_id, ip.patient_id
      FROM clinic_row c
      INNER JOIN (
        VALUES
          (1, 'CL11-WEEK-A-1', '2026-04-27', '2026-04-27', 210, 200, 10),
          (2, 'CL11-WEEK-A-2', '2026-04-28', '2026-04-28', 325, 300, 25),
          (3, 'CL11-WEEK-A-3', '2026-04-29', '2026-04-29', 160, 160, 0),
          (4, 'CL11-WEEK-A-4', '2026-05-01', '2026-05-01', 280, 260, 20),
          (5, 'CL11-WEEK-A-5', '2026-05-03', '2026-05-03', 245, 230, 15)
      ) AS seed(patient_rn, "tag", "startDate", "endDate", "calculatedFee", "chargedFee", "discountFee")
        ON true
      INNER JOIN indexed_patients ip ON ip.rn = seed.patient_rn
      WHERE NOT EXISTS (
        SELECT 1
        FROM "Appointment" a
        WHERE a."clinicId" = c.clinic_id
          AND a."patient" = ip.patient_id
    ...
    await queryRunner.query(`
      ...
      FROM (
        VALUES
          ...
          (2, 2), (4, 2), (5, 2),
          (1, 3), (5, 3), (6, 3)
      ) AS seed(treatment_rn, dentist_rn)
    ...
    await queryRunner.query(`
      ...
      appointment_rows AS (
        SELECT a.id AS appointment_id, a."patient", ROW_NUMBER() OVER (ORDER BY a."startDate", a.id) AS rn
        FROM "Appointment" a
      ...
      SELECT p.patient_id, pt.tooth, a.appointment_id, tr.treatment_id, d.dentist_id,
             seed.fee_snapshot, seed.description
      FROM (
        VALUES
          (1, 1, 1, 1, 75,  'CL11-WEEK-TT-1'),
          (1, 2, 3, 2, 135, 'CL11-WEEK-TT-2'),
          (2, 1, 2, 2, 110, 'CL11-WEEK-TT-3'),
          (2, 2, 4, 2, 190, 'CL11-WEEK-TT-4'),
          (3, 1, 5, 3, 160, 'CL11-WEEK-TT-5'),
          (3, 2, 1, 1, 85,  'CL11-WEEK-TT-6'),
          (4, 1, 2, 2, 130, 'CL11-WEEK-TT-7'),
          (4, 2, 4, 3, 150, 'CL11-WEEK-TT-8'),
          (5, 1, 6, 1, 95,  'CL11-WEEK-TT-9'),
          (5, 2, 5, 3, 140, 'CL11-WEEK-TT-10')
      ...
      INNER JOIN appointment_rows a ON a.rn = seed.patient_rn AND a."patient" = p.patient_id
      INNER JOIN patient_teeth pt ON pt.patient = p.patient_id AND pt.tooth_rn = seed.tooth_rn
      INNER JOIN treatment_rows tr ON tr.rn = seed.treatment_rn
      INNER JOIN dentist_rows d ON d.rn = seed.dentist_rn
      WHERE NOT EXISTS (
        SELECT 1
    ...
    await queryRunner.query(`
      WITH tt_rows AS (
        ...
        WHERE description LIKE 'CL11-WEEK-TT-%'
      )
    ...
    await queryRunner.query(`
      ...
      tt_rows AS (
        ...
        FROM "Tooth_Treatment" tt
        WHERE tt.description LIKE 'CL11-WEEK-TT-%'
      )
      ...
      FROM (
        VALUES
          (1, 1, 1, '2026-04-27 09:00:00', '2026-04-27 09:30:00', 'CL11-WEEK-RV-1'),
          (2, 2, 2, '2026-04-28 10:00:00', '2026-04-28 10:45:00', 'CL11-WEEK-RV-2'),
          (3, 3, 3, '2026-04-29 11:00:00', '2026-04-29 11:35:00', 'CL11-WEEK-RV-3'),
          (4, 1, 1, '2026-05-01 12:30:00', '2026-05-01 13:10:00', 'CL11-WEEK-RV-4'),
          (5, 2, 2, '2026-05-03 14:00:00', '2026-05-03 14:40:00', 'CL11-WEEK-RV-5'),
          (6, 3, 3, '2026-05-03 15:00:00', '2026-05-03 15:30:00', 'CL11-WEEK-RV-6')
    ...
    await queryRunner.query(`
      ...
      rv_rows AS (
        ...
        FROM "Randevue" rv
        WHERE rv.note LIKE 'CL11-WEEK-RV-%'
      )
