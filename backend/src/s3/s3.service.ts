...
export class S3Service {
    ...
    async uploadFile(file: Express.Multer.File, key: string): Promise<string> {
        try {
            this.logger.debug(`Uploading file ${file.originalname} to S3 with key ${key}`);
            LogWriter.append('debug', S3Service.name, `Uploading file ${file.originalname} to S3 with key ${key}`);
            ...
            LogWriter.append('log', S3Service.name, `File uploaded successfully: ${url}`);
