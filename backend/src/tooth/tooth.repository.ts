...
export class ToothRepository {
  ...
  async findTeethWithFilters(
    ...
    if (filters.number !== undefined) {
      queryBuilder.andWhere('tooth.number = :number', {
        number: filters.number,
      });
    }
    if (filters.permanent !== undefined) {
      queryBuilder.andWhere('tooth.permanent = :permanent', {
        permanent: filters.permanent,
      });
    }
    if (filters.upperJaw !== undefined) {
      queryBuilder.andWhere('tooth.upper_jaw = :upperJaw', {
        upperJaw: filters.upperJaw,
      });
    }
